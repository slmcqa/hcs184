...
describe "New_Philhealth_Case_rate_Inpatient" do
  ...
  it "TEST001 - EXEMPTED SPC RULE - CATARACT SURGERY - Create, Admit and Order items" do
  ...
  it "TEST001 - EXEMPTED SPC RULE - CATARACT SURGERY - Get Gross Order " do
    @@comp_drugs = 0
    @@comp_xray_lab = 0
    @@comp_operation = 0
    @@comp_others = 0
  ...
  it "TEST001 - EXEMPTED SPC RULE - CATARACT SURGERY - Compute Philhealth" do
  ...
  it "TEST001 - EXEMPTED SPC RULE - CATARACT SURGERY - Verify Case Rate And PF Fee" do
     @@mycase_rate =  "66983"
     Database.connect
            t = "SELECT TO_CHAR(PF_AMOUNT) FROM SLMC.REF_PBA_PH_CASE_RATE WHERE RVS_CODE ='#{@@mycase_rate}'"
            pf = Database.select_last_statement t
     Database.logoff
     Database.connect
            t = "SELECT TO_CHAR(RATE)  FROM SLMC.REF_PBA_PH_CASE_RATE WHERE RVS_CODE ='#{@@mycase_rate}'"
...
#     @@non_comp_supplies
  ...
  it "TEST001 - EXEMPTED SPC RULE - CATARACT SURGERY - Checks if the actual charge for drugs/medicine is correct" do
  ...
  it "TEST001 - EXEMPTED SPC RULE - CATARACT SURGERY - Checks if the actual charge for xrays, lab and others is correct" do
  ...
  it "TEST001 - EXEMPTED SPC RULE - CATARACT SURGERY - Checks if the actual charge for operation is correct" do
  ...
  it "TEST001 - EXEMPTED SPC RULE - CATARACT SURGERY - Checks if the actual charge for room and board is correct" do
    @@days1 = 1.0
    @@actual_room_charges = (@@room_discount * @@days1)
    @@ph1[:room_and_board_actual_charges].should == ("%0.2f" %(@@actual_room_charges))
  end
  it "TEST001 - EXEMPTED SPC RULE - CATARACT SURGERY - Checks if the total actual charge(s) is correct" do
  ...
  it "TEST001 - EXEMPTED SPC RULE - CATARACT SURGERY - Settle Balance Due" do
    sleep 10
...
#    (@@discount_amount + @@total_actual_charges)
  ...
  it "TEST001 - EXEMPTED SPC RULE - CATARACT SURGERY  - Print Gatepass" do
       ######## Print Gate Pass
       sleep 10
  ...
  it "TEST002 - EXEMPTED SPC RULE - NON CATARACT SURGERY  - Create, Admit and Order items  And Verify Total PH Claims" do
  ...
  #  @@pin1 = "1210068782"
    ...
    @@mycase_rate =  "11720"
     Database.connect
            t = "SELECT PF_AMOUNT FROM SLMC.REF_PBA_PH_CASE_RATE WHERE RVS_CODE ='#{@@mycase_rate}'"
            pf = Database.select_last_statement t
     Database.logoff
     Database.connect
            t = "SELECT RATE  FROM SLMC.REF_PBA_PH_CASE_RATE WHERE RVS_CODE ='#{@@mycase_rate}'"
            rate = Database.select_last_statement t
     Database.logoff
      puts rate
...
slmc.my_pba_full_payment


    end
  it "TEST003 - EXEMPTED SPC RULE - CATARACT SURGERY - MORE THAN 1 DAY AND LESS THAN 2 DAY - Create, Admit and Order items  And Verify Total PH Claims" do
 #   @@visit_no1 = "5510000716"
            days_to_adjust = 46
            discharge = 1
            ...
            mydischarge = ((d - discharge).strftime("%m/%d/%Y").upcase).to_s
            puts "my_discharge_date #{mydischarge}"            
            Database.connect
                   #@@visit_no1 = "5409000018"
                  visit_no = @@visit_no1
                   plsql.connection = Database.connect
                              a = plsql.slmc.sproc_updater(visit_no,my_set_date,mydischarge)
            #       plsql.logoff
            Database.logoff
            puts a
#####            @@pin1 = "1409075134"
            ...
            slmc.nursing_gu_search(:pin => @@pin1)
            @@room_and_bed = slmc.get_room_and_bed_no_in_gu_page
            @@visit_no2 = slmc.clinically_discharge_patient(:pin => @@pin1, :diagnosis => "A91.0", :no_pending_order => true, :pf_amount => "3000", :save => true).should be_true
            puts @@pin1
            ...
            sleep 6
            slmc.is_text_present("Selected 1st case rate is invalid.")
  ...
  it "TEST004 - EXEMPTED SPC RULE - NON CATARACT SURGERY - MORE THAN 1 DAY AND LESS THAN 2 DAY - Verify Total PH Claims" do
  ...
  it "TEST005 - EXEMPTED SPC RULE - CATARACT SURGERY - MORE THAN 2 DAY - Create, Admit and Order items  And Verify Total PH Claims" do
            slmc.login(@pba_user, @password)
 ...
 #          @@pin1 = "1409075195"
            ...
            slmc.skip_update_patient_information.should be_true
            slmc.skip_room_and_bed_cancelation.should be_true
            @@ph1 = slmc.philhealth_computation(:claim_type => "ACCOUNTS RECEIVABLE", :diagnosis => "CHOLERA", :medical_case_type => "ORDINARY CASE", :with_operation => true, :rvu_code => "66983", :compute => true, :cancel=> true)
            slmc.ph_save_computation.should be_true
            @@mycase_rate =  "66983"
            Database.connect
                    t = "SELECT TO_CHAR(PF_AMOUNT) FROM SLMC.REF_PBA_PH_CASE_RATE WHERE RVS_CODE ='#{@@mycase_rate}'"
...
###### Verify payment page
  ...
  it "TEST006 - EXEMPTED SPC RULE - NON CATARACT SURGERY - MORE THAN 2 DAY" do
            slmc.login(@pba_user, @password)
            slmc.go_to_patient_billing_accounting_page
            slmc.pba_search(:with_discharge_notice => true, :pin => @@pin1)
            slmc.go_to_page_using_visit_number("Discharge Patient", slmc.visit_number)
            ...
            slmc.skip_update_patient_information.should be_true
            slmc.skip_room_and_bed_cancelation.should be_true
            @@ph1 = slmc.philhealth_computation(:claim_type => "ACCOUNTS RECEIVABLE", :diagnosis => "CHOLERA", :medical_case_type => "ORDINARY CASE", :with_operation => true, :rvu_code => "11720", :compute => true, :cancel=> true)
            slmc.ph_save_computation.should be_true
            @@mycase_rate =  "11720"
            Database.connect
                    t = "SELECT TO_CHAR(PF_AMOUNT) FROM SLMC.REF_PBA_PH_CASE_RATE WHERE RVS_CODE ='#{@@mycase_rate}'"
  ...
  it "TEST007 - NON EXEMPTED SPC RULE -  CATARACT SURGERY - NO DATA " do
    ############ NO DATA ###############
    ############ NO CASE RATE FOR NON EXEMPTED SPC RULE AND CATARACT SURGERY ##################
  end
  it "TEST008 - NON EXEMPTED SPC RULE -  NON CATARACT SURGERY - Create, Admit and Order items" do
              ...
              slmc.login(@user, @password)
              slmc.admission_search(:pin => "Test")
              @@pin3 = slmc.create_new_patient(@patient2).gsub(' ', '')
              #@@pin2 = "1210068782"
              ...
              @@room_and_bed = slmc.get_room_and_bed_no_in_gu_page
              @@visit_no3 = slmc.clinically_discharge_patient(:pf_type => "COLLECT",:pin => @@pin3, :diagnosis => "A91.0", :no_pending_order => true, :pf_amount => "1008", :save => true).should be_true
              puts @@visit_no3
    end
  it "TEST008 - NON EXEMPTED SPC RULE -  NON CATARACT SURGERY - Get Gross Order " do
    @@comp_drugs = 0
    @@comp_xray_lab = 0
    @@comp_operation = 0
    @@comp_others = 0
  ...
  it "TEST008 - NON EXEMPTED SPC RULE -  NON CATARACT SURGERY - Compute Philhealth" do
          slmc.login(@pba_user, @password)
          slmc.go_to_patient_billing_accounting_page
          ...
          slmc.skip_update_patient_information.should be_true
          slmc.skip_room_and_bed_cancelation.should be_true
          @@ph2 = slmc.philhealth_computation(:claim_type => "ACCOUNTS RECEIVABLE", :diagnosis => "CHOLERA", :medical_case_type => "ORDINARY CASE", :with_operation => true, :rvu_code => "11444", :compute => true)
          slmc.ph_save_computation.should be_true
  end
  it "TEST008 - NON EXEMPTED SPC RULE -  NON CATARACT SURGERY - Verify Case Rate And PF Fee" do
     @@mycase_rate =  "11444"
     Database.connect
            t = "SELECT TO_CHAR(PF_AMOUNT) FROM SLMC.REF_PBA_PH_CASE_RATE WHERE RVS_CODE ='#{@@mycase_rate}'"
            pf = Database.select_last_statement t
     ...
     Database.connect
            ...
            rate = Database.select_last_statement t
     Database.logoff
     mm = @@ph2[:total_actual_benefit_claim].to_f
     puts mm
     puts rate
     puts pf
     pf = pf.to_i
...
#     @@non_comp_supplies
     ...
     ((slmc.truncate_to((@@ph2[:inpatient_physician_benefit_claim].to_f - @@physician_pf_claim),2).to_f).abs).should <= 0.01
  end
  it "TEST008 - NON EXEMPTED SPC RULE -  NON CATARACT SURGERY - Checks if the actual charge for drugs/medicine is correct" do
    total_drugs = @@comp_drugs + @@non_comp_drugs
    @@actual_medicine_charges = total_drugs - (@@promo_discount * total_drugs)
    @@ph2[:actual_medicine_charges].should == ("%0.2f" %(@@actual_medicine_charges))
  end
  it "TEST008 - NON EXEMPTED SPC RULE -  NON CATARACT SURGERY - Checks if the actual charge for xrays, lab and others is correct" do
    total_xrays_lab_others = @@comp_xray_lab + @@non_comp_xray_lab + @@non_comp_supplies
    @@actual_xray_lab_others = total_xrays_lab_others - (total_xrays_lab_others * @@promo_discount)
    @@ph2[:actual_lab_charges].should == ("%0.2f" %(@@actual_xray_lab_others))
  end
  it "TEST008 - NON EXEMPTED SPC RULE -  NON CATARACT SURGERY - Checks if the actual charge for operation is correct" do
    @@actual_operation_charges = @@comp_operation - (@@comp_operation * @@promo_discount) + (@@non_comp_operation - (@@non_comp_operation * @@promo_discount))
    @@ph2[:actual_operation_charges].should == ("%0.2f" %(@@actual_operation_charges))
  end
  it "TEST008 - NON EXEMPTED SPC RULE -  NON CATARACT SURGERY - Checks if the actual charge for room and board is correct" do
    @@days1 = 1.0
    @@actual_room_charges = (@@room_discount * @@days1)
    @@ph2[:room_and_board_actual_charges].should == ("%0.2f" %(@@actual_room_charges))
  end
  it "TEST008 - NON EXEMPTED SPC RULE -  NON CATARACT SURGERY - Checks if the total actual charge(s) is correct" do
    @@total_actual_charges = @@actual_medicine_charges + @@actual_xray_lab_others + @@actual_operation_charges + @@actual_room_charges
    ((slmc.truncate_to((@@ph2[:total_actual_charges].to_f - @@total_actual_charges),2).to_f).abs).should <= 0.01
  end
  it "TEST008 - NON EXEMPTED SPC RULE -  NON CATARACT SURGERY - Settle Balance Due" do
    sleep 10
    slmc.skip_philhealth.should be_true
    slmc.skip_discount.should be_true
    slmc.skip_generation_of_soa.should be_true
...
#    (@@discount_amount + @@total_actual_charges)
  ...
  it "TEST008 - NON EXEMPTED SPC RULE -  NON CATARACT SURGERY - Print Gatepass" do
       ######## Print Gate Pass
       slmc.login(@gu_user_0287, @password).should be_true
       slmc.nursing_gu_search(:pin => @@pin3)
  ...
  it "TEST009 - NON EXEMPTED SPC RULE -  CATARACT SURGERY - MORE THAN 90 DAYS -  NO DATA " do
        ############ NO DATA ###############
       ############ NO CASE RATE FOR NON EXEMPTED SPC RULE AND CATARACT SURGERY ##################
  end
  it "TEST010 - NON EXEMPTED SPC RULE -  NON CATARACT SURGERY - MORE THAN 90 DAYS" do
            days_to_adjust = 91
            d = Date.strptime(Time.now.strftime('%Y-%m-%d'))
            #my_set_date = ((d - days_to_adjust).strftime("%d/%m/%Y").upcase).to_s
            my_set_date = ((d - days_to_adjust).strftime("%m/%d/%Y").upcase).to_s
...
####            @@pin1 = "1409075134"
            slmc.login(@gu_user_0287, @password)
            slmc.nursing_gu_search(:pin => @@pin3)
            slmc.print_gatepass(:no_result => true, :pin => @@pin3)
            slmc.login(@user, @password)
            slmc.admission_search(:pin => @@pin3).should be_true
            slmc.create_new_admission(:account_class => "INDIVIDUAL", :org_code => "0287", :rch_code => "RCH08",
              :room_charge => "REGULAR PRIVATE", :diagnosis => "DENGUE FEVER", :doctor_code => "6726").should == "Patient admission details successfully saved."
            ...
            slmc.nursing_gu_search(:pin => @@pin3)
            @@room_and_bed = slmc.get_room_and_bed_no_in_gu_page
            @@visit_no3 = slmc.clinically_discharge_patient(:pin => @@pin3, :diagnosis => "A91.0", :no_pending_order => true, :pf_amount => "1008", :save => true).should be_true
            puts @@pin3
